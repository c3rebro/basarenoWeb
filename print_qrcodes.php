...
$stmt = $conn->prepare("SELECT * FROM products WHERE seller_number=?");
...
<table class="outer-table">
    ...
    <tr>
            ...
            <script nonce="<?php echo $nonce; ?>">
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    QRCode.toCanvas(canvas, "<?php echo htmlspecialchars($product['barcode'], ENT_QUOTES, 'UTF-8'); ?>", {
                        ...
                        if (error) console.error("QR code generation failed for barcode: <?php echo htmlspecialchars($product['barcode']); ?>", error);
                    });
                });
            </script>
        </td>
        ...
        <?php else: ?>
            <td style="padding: 4px;"></td> <!-- Empty cell if size is not available -->
        <?php endif; ?>
        <td></td>
    </tr>
    <tr>
    ...
    <tr>
        <!-- Row 4: Seller and ID -->
        <td style="padding: 4px;">
            Verk.Nr.: <?php echo htmlspecialchars($seller_number); ?>
        </td>
        <td style="padding: 4px;">
            (I.: <?php echo htmlspecialchars($seller['checkout_id']); ?> - <?php echo htmlspecialchars($product['product_id']); ?>)
        </td>
    </tr>
</table>
            </td>
