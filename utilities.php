...
/**
 * Return true if the bazaar still has free seller slots.
 */
function bazaar_has_free_slots(mysqli $conn, int $bazaarId): bool {
    $sql = "
        SELECT 
            b.max_sellers,
            (SELECT COUNT(*) FROM sellers s WHERE s.bazaar_id = b.id AND s.seller_verified = 1) AS current_sellers
        FROM bazaar b
        WHERE b.id = ?
        LIMIT 1";
    $stmt = $conn->prepare($sql);
    ...
    $stmt->execute();
    $row = $stmt->get_result()->fetch_assoc();
    if (!$row) return false; // unknown bazaar -> act as full
    return (int)$row['current_sellers'] < (int)$row['max_sellers'];
}
